    ...
    private function setMultisiteReplaceList()
    {
        ...
        switch ($this->post['action_mu_mode']) {
            ...
            case DUPX_MultisiteMode::Standalone:
                ...
                $search  = $archive_config->getUrlFromSubsiteObj($standalone_obj);
                $replace = $this->post['url_new'];
                $s_r_manager->addItem($search, $replace, DUPX_S_R_ITEM::TYPE_URL_NORMALIZE_DOMAIN, DUPX_UpdateEngine::SR_PRORITY_NETWORK_SUBSITE);
    ...
    public function configurationFileUpdate()
    {
        ...
        try {
            if (file_exists(DUPX_Package::getWpconfigArkPath())) {
                $this->configurationUrlsAndPaths();
...
LONGMSG;
    ...
    protected function configurationUrlsAndPaths()
    {
    ...
    public function htaccessUpdate()
    {
        ...
        if (!empty($new_htaccess_name)) {
            if (DUPX_ServerConfig::renameHtaccess($this->post['path_new'], $new_htaccess_name)) {
                DUPX_Log::info("\nReset original .htaccess content from .htaccess__[HASH]");
            }
        }
